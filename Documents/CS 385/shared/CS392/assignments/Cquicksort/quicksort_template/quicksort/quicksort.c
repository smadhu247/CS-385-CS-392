...
static int lomuto(void *array, int left, int right, size_t elem_sz, int (*comp) (const void*, const void*)) {
    char* temp_array = (char *) array;
    char* p = (temp_array + (left * elem_sz));
    size_t s = left;
    for(size_t i = left + 1; i <= right; i++){
    
      if (comp((temp_array + (i *elem_sz)), p) < 0){  
        s = s + 1;
        swap((temp_array + (s * elem_sz)), (temp_array + (i * elem_sz)), elem_sz);
      }
    }
    swap((temp_array + (left *elem_sz)), (temp_array + (s * elem_sz)), elem_sz);
    return s;
}
