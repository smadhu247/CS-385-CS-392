...
private:
    ...
    void delete_tree(Node<K, V> *n) {
        //deletes tree by recursively traversing the left and right subtree until all nodes have been deleted
    	if(n != nullptr){
    		if (n->left != nullptr) {
				delete_tree(n->left);
			}
    ...
    int height(Node<K, V> *node) const {
    	//if the root is null, the height of the tree is -1
    	if(node == nullptr){
    		return -1;
    	}
        if(node == root_){
        ...
        //recursively traverses the right and left subtree to find max height
    ...
    size_t leaf_count(Node<K, V> *node) const {
    	int count = 0;
    	//base case: when the node is null, we return the count
        if(node == nullptr){
        	return 0;
        }
        else {
        	//if the node has no children, then we increment the count
        	if(node->left == nullptr && node->right == nullptr){
        		count++;
        	}
        	//search the tree by calling the method recursively on its children
        	count = count + leaf_count(node->left);
        	count = count + leaf_count(node->right);
        }
        return count;
    }
    ...
    size_t internal_node_count(Node<K, V> *node) const {
		...
		if(node == nullptr){
			...
			//if the node has children, then we increment the count
			if(node->left != nullptr || node->right != nullptr){
				count++;
			}
			//search the tree by calling the method recursively on its children
			count = count + internal_node_count(node->left);
			count = count + internal_node_count(node->right);
		}
		return count;
    }
    ...
    int diameter(Node<K, V> *node) const {
    	...
    	//finds the right path, left path and adds three extra nodes to account for the starting left
    	//and starting right path node as well as the current node itself.
    	return height(node->right) + height(node->left) + 3;
    }
    ...
    size_t width(Node<K, V> *node, size_t level) const {
    	...
    	//special case where the level is 0, then we know the width is 1
    	if (level == 0){
    		return 1;
    	}
    	else{
    		//recursively calls the left and right child with the method and decremented level
    		level = level - 1;
    		return width(node->left, level) + width(node->right, level);
    	}
    }
    ...
    size_t null_count(Node<K, V> *node) const {
		...
		if(node == nullptr){
			...
			//increments the count on the left and right children
			count = count + null_count(node->left);
			count = count + null_count(node->right);
		}
		return count;
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
    	//if we encounter a non null node, then increment level and recursively call on left and right child
    	if (node != nullptr) {
    		level = level + 1;
    		return (level - 1) + sum_levels(node->left, level) + sum_levels(node->right, level);
    	}
    	return 0;
    }
